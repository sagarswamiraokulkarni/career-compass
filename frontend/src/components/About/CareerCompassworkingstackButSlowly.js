    ...
    useEffect(() => {
        if (wrapperRef.current && cardsRef.current.length > 0 && leftContentRef.current) {
            ...
            gsap.set(cards, {
                ...
                zIndex: (i, target) => i + 1, // Ensure proper stacking order
            });
            ...
            cards.forEach((card, index) => {
                if (index > 0) { // Skip the first card as it's already at the top
                    tl.to(card, {
                        y: 0,
                        duration: 1,
                        ease: "power1.inOut",
                    }, (index - 1) / (numCards - 1)); // Stagger the animations
                }
            });
        ...
        <>
            ...
            <section className="stackable-cards-section" ref={sectionRef}>
                ...
                <div className="right-content" ref={wrapperRef}>
                    <div className="cards">
                        {['DAY 1: Find your Objective & TG', 'DAY 2: Profile Optimisation', 'DAY 3: Another Topic'].map((text, index) => (
