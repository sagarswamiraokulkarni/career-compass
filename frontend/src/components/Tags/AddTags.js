    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await Promise.all(addedTags.map(async (tag) => {
                const response = await careerCompassApi.postApiCall(userJson, urlPaths.CREATE_TAG, {
                    name: tag,
                ...
                {addedTags.length > 0 && (
                    ...
                    {tag}
                                    <button className="remove-tag-btn"
                                            onClick={() => handleRemoveTag(tag, "new")}>x</button>
                </span>
                            ))}
                        </div>
                    </div>
                ...
                {deletedTags.length > 0 && <div className="added-tags-container">
                    ...
                    <div className="added-tag-list">
                        {deletedTags.map((tag, index) => (
                            ...
                            {tag}
                                <button className="remove-tag-btn"
                                        onClick={() => handleRemoveTag(tag, "delete")}>x</button>
                        </span>
                        ))}
                    </div>
                </div>}
