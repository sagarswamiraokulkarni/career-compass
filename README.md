...
public NotificationStrategy getNotificationStrategy(String strategyType){
    return switch (strategyType) {
        case "sms" -> new MessageNotificationStrategy("sms");
        case "whatsapp" -> new MessageNotificationStrategy("whatsapp");
...
## Plan for Completion:
- Still about 20% of the work is left to be done wherein we wish to implement one more pattern and extend the functionalities of the job application dashboard.

- The plan for the next iteration is as below:
  - To implement a factory pattern to simplify the instantiation of different NotificationStrategy objects such as Email, SMS and WhatsApp.
  - To extend functionalities to view archived job applications and also unarchive the archived job applications.
